# Constraints: `pr-babysitter` (the constitution)
...
- Never push to `main` — this loop makes no commits beyond its own three owned
  files (`pr-status-report.md`, `pr-babysitter-state.md`, `loop-run-log.md`); it
  never opens a branch, comments, or pushes to any PR.
