# Constraints: `post-merge-cleanup` (the constitution)
...
- Never push to `main` — this loop makes no commits beyond its own three owned
  files (`cleanup-report.md`, `post-merge-cleanup-state.md`, `loop-run-log.md`);
  it never deletes a branch, closes an issue, or tears down an environment.
