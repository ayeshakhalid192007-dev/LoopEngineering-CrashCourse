...
Medium cost per the catalog (a full-route benchmark run every beat).
...
- Fleet token ceiling: this kit is a standalone starter — set its own cap in
  ...
  share a fleet total.
- Priority when the fleet is over budget: performance work is valuable but
  rarely urgent — pause this loop before any correctness or security loop if
  the fleet is burning too fast.
