# Constraints: `issue-triage` (the constitution)
...
- Never push to `main` — this loop makes no commits beyond its own three owned
  files (`issue-triage-report.md`, `issue-triage-state.md`, `loop-run-log.md`);
  it never labels, closes, or comments on an issue.
