...
| --- | --- | --- |
| Runs / day | 12 | drop to report-only at 9 runs |
...
- Fleet token ceiling: this kit is a standalone starter — set its own cap in
  ...
  share a fleet total.
- Priority when the fleet is over budget: report loops like this one pause
  after any write-capable loop in the same fleet, since a missed triage
  report costs a re-read, not correctness.
