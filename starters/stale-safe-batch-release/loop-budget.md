# Budget: `stale-safe-batch-release`
...
- Fleet token ceiling: this kit is a standalone starter — set its own cap in
  ...
  share a fleet total.
- Priority when the fleet is over budget: release batching is Low cost and
  runs only per release window — pause it before any higher-cost, more
  frequent loop if the fleet is burning too fast.
