...
| **Dropped events assumed queued** | gaps during bursts, zero errors | reconciliation sweep ([Step 7](../part-2-heartbeat/07-event-driven.md)) |
...
| **Uncommitted spine** | state fine locally, gone when needed most | commit every beat — this repo's Day 1 scar ([Step 12](../part-4-the-spine/12-state-between-runs.md)) |
...
| Failure | The tell | The fix |
| --- | --- | --- |
| **Green ≠ done** | statuses perfect, product broken | outcome checks that drive the flow ([verification](../part-6-human-control/verification.md)) |
...
| **Zombie loop** | runs forever, influences nothing | economics test ([decision framework](../methods/decision-framework.md)); retire it |
