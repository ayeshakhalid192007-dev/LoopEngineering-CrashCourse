...
1. Read `shared/loop-budget.md` for daily caps and kill-switch flags.
2. Read recent entries in `shared/loop-run-log.md` (last 24h).
...
5. If spend ≥ 100% or `loop-pause-all` is set → **exit immediately** with a one-line note in your loop’s own state.md.
6. If watchlist/state has no actionable items → **exit in <5k tokens** (do not spawn sub-agents).

## End of run

Append one JSON object to `shared/loop-run-log.md`:
...
- Never exceed `max sub-agent spawns/run` from `shared/loop-budget.md`.
- High-cadence patterns (CI Sweeper, PR Babysitter) **must** early-exit when nothing is actionable.
- On self-throttle, append a line to `shared/loop-budget.md` under **Alerts This Period**.